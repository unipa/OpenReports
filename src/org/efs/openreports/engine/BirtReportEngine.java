 ...
 * 2011 FRANCESCO
 * Aggiunto:
 * 1) Supporto per i Pool di connessione
 * 2) Emissionee di file CSV, PPT, ODP, ODS, ODT, POSTSCRIPT
 * 
...
public class BirtReportEngine extends ReportEngine
{
			...
			throws ProviderException
	{
		...
		// Set default RenderOption options for task
		...
		try
		{
			...
			if (input.getExportType() == ExportType.PDF){
				...
				PDFRenderOption renderOptionPDF = new PDFRenderOption();
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_PDF);
				renderOptionPDF.setOutputFormat(IRenderOption.OUTPUT_FORMAT_PDF);
				renderOptionPDF.setOutputStream(out);
				task.setRenderOption(renderOptionPDF);
			}
			...
			{
				EXCELRenderOption renderOptionExcel = new EXCELRenderOption();	
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_XLS);
				renderOptionExcel.setOutputFormat("xls");
				renderOptionExcel.setOutputStream(out);
				task.setRenderOption(renderOptionExcel);
				
			}
			else if (input.getExportType() == ExportType.CSV)
			{	
				CSVRenderOption renderOptionCSV = new CSVRenderOption();	
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_CSV); 
				renderOptionCSV.setOutputFormat(CSVRenderOption.OUTPUT_FORMAT_CSV);	
				renderOptionCSV.setDelimiter(";");
				renderOptionCSV.setOutputStream(out);
				task.setRenderOption(renderOptionCSV);				
			}				
			else if (input.getExportType() == ExportType.RTF){
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_DOC); 
				renderOption.setOutputFormat("doc");
				task.setRenderOption(renderOption);
			}
			else if (input.getExportType() == ExportType.PPT)
			{	
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_PPT); 
				renderOption.setOutputFormat("ppt");
				task.setRenderOption(renderOption);
			}
			else if (input.getExportType() == ExportType.ODT)
			{  			
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_ODT); 
				renderOption.setOutputFormat("odt");	
				task.setRenderOption(renderOption);
			}
			else if (input.getExportType() == ExportType.ODP)
			{			
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_ODP); 
				renderOption.setOutputFormat("odp");		
				task.setRenderOption(renderOption);
			}
			else if (input.getExportType() == ExportType.ODS)
			{			
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_ODS); 
				renderOption.setOutputFormat("ods");	
				task.setRenderOption(renderOption);
			}
			else if (input.getExportType() == ExportType.PS)
			{			 
				output.setContentType(ReportEngineOutput.CONTENT_TYPE_PS); 
...
//			if (input.getExportType() == ExportType.RTF) {
//				String strOut=out.toString();
//				System.out.println("******************** BirtReportEngine.generateReport() - strOut"+strOut);
//			}
					...
					{		
						...
						try
						{
							if(reportDataSource.isJndi())
							{
								...
								dataSH.setStringProperty("odaDriverClass", null);
								dataSH.setStringProperty("odaUser", null);
								dataSH.setStringProperty("odaPassword", null);							
							}else
							{							
								dataSH.setStringProperty("odaJndiName", null);
